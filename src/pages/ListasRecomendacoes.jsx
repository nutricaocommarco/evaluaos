...
const CAMPOS_VAZIOS = { titulo: 'Lista', conteudo: '', salvarComoModelo: false, imagem_url: null }
...
export default function ListasRecomendacoes({ userId }) {
  ...
  const abrirEdicaoLista = (l) => {
    setEditingId(l.id)
    setForm({ titulo: l.titulo || 'Lista', conteudo: l.conteudo || '', salvarComoModelo: false, imagem_url: l.imagem_url || null })
    setShowModal(true)
  }
                    ...
                    <div key={l.id} className="bg-white dark:bg-slate-900 rounded-xl border border-gray-100 dark:border-slate-800 shadow-sm overflow-hidden">
                      ...
                      {aberto && (
                        <div className="px-4 pb-4 space-y-3">
                          {l.imagem_url && (
                            <img src={l.imagem_url} alt="" className="w-full max-w-xs rounded-xl border border-gray-100 dark:border-slate-800 object-cover" />
                          )}
                          {l.conteudo ? (
