                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex flex-wrap gap-1">
                          {colunas.map((r) => {
                            const itens = registrosPorCelula[`${dataISO}__${r.nome_refeicao}`] || []
                            ...
                            const cor = r.semMeta ? 'bg-purple-400' : dentro ? 'bg-emerald-500' : 'bg-red-500'
                            return (
                              <button
                                key={r.nome_refeicao}
                                onClick={() => setCelulaSelecionada(ativa ? null : { dia, nomeRefeicao: r.nome_refeicao })}
                                title={`${r.nome_refeicao}: ${Math.round(kcalReal)} kcal${r.semMeta ? '' : ` · meta ${Math.round(r.kcalPrescrito)} kcal`}`}
                                className={`w-3 h-3 rounded-full ${cor} transition-transform ${ativa ? 'ring-2 ring-offset-1 ring-primary-500 scale-125' : ''}`}
                              />
                            )
                          })}
                        </div>
