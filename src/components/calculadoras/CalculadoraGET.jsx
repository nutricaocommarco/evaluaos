...
export default function CalculadoraGET({ formData, setFormData, results, setResults, setPlannerResults }) {
  ...
  return (
    <div className="bg-slate-50 rounded-[1.25rem] sm:rounded-[2rem] md:rounded-[3.5rem] p-3 sm:p-8 md:p-12 border border-slate-200 shadow-inner">
      <h2 className="text-xl md:text-3xl font-black text-slate-800 uppercase italic mb-6 md:mb-8 border-b border-emerald-200 pb-3 md:pb-4 flex items-center gap-2 md:gap-3">
        <Calculator className="text-emerald-700 w-5 h-5 md:w-8 md:h-8 flex-shrink-0"/> Calculadora de Gasto Calórico
      </h2>
      
      <form onSubmit={handleCalculate} className="space-y-6 md:space-y-10">
        {/* ETAPA 1 */}
        <section>
          <h3 className="text-base md:text-lg font-black text-slate-800 uppercase italic mb-4 flex items-center gap-2">
            <User className="text-emerald-700 w-4 h-4 md:w-5 md:h-5 flex-shrink-0" /> 1. Parâmetros do Paciente
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 px-1">
            <div>
              <label className="block text-[10px] md:text-xs font-bold text-slate-700 mb-1 md:mb-2 uppercase">Sexo</label>
              <select name="gender" value={formData.gender} onChange={handleInputChange} className="w-full p-2.5 md:p-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 bg-white font-medium outline-none text-sm">
                <option value="M">Masculino</option>
                <option value="F">Feminino</option>
              </select>
            </div>
            <div>
              <label className="block text-[10px] md:text-xs font-bold text-slate-700 mb-1 md:mb-2 uppercase">Idade (anos)</label>
              <input type="number" name="age" value={formData.age} onChange={handleInputChange} required className="w-full p-2.5 md:p-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none text-sm" />
            </div>
            <div>
              <label className="block text-[10px] md:text-xs font-bold text-slate-700 mb-1 md:mb-2 uppercase">Peso (kg)</label>
              <input type="number" step="0.1" name="weight" value={formData.weight} onChange={handleInputChange} required className="w-full p-2.5 md:p-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none text-sm" />
            </div>
            <div>
              <label className="block text-[10px] md:text-xs font-bold text-slate-700 mb-1 md:mb-2 uppercase">Altura (cm)</label>
              <input type="number" name="height" value={formData.height} onChange={handleInputChange} required className="w-full p-2.5 md:p-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none text-sm" />
            </div>
            <div className="md:col-span-2">
              <label className="block text-[10px] md:text-xs font-bold text-slate-700 mb-1 md:mb-2 uppercase">% Gordura <span className="text-slate-400 normal-case block sm:inline mt-0.5 sm:mt-0">- Para hipertrofia e atletas</span></label>
              <input type="number" step="0.1" name="bf" value={formData.bf} onChange={handleInputChange} className="w-full p-2.5 md:p-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none text-sm" />
            </div>
          </div>
        </section>
        ...
        <section>
          <h3 className="text-base md:text-lg font-black text-slate-800 uppercase italic mb-4 flex items-center gap-2">
            <Activity className="text-emerald-700 w-4 h-4 md:w-5 md:h-5 flex-shrink-0" /> 2. Perfil Biológico
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 md:gap-3 px-1">
            {[
              ...
              <label key={item.id} className={`p-2.5 md:p-3 border-2 rounded-xl cursor-pointer text-center text-sm md:text-base ${formData.bodyType === item.id ? 'border-emerald-600 bg-emerald-50 text-emerald-800 font-bold' : 'border-slate-200 text-slate-600 font-bold hover:bg-white'}`}>
        ...
        <section>
          <h3 className="text-base md:text-lg font-black text-slate-800 uppercase italic mb-4 flex items-center gap-2">
            <HeartPulse className="text-emerald-700 w-4 h-4 md:w-5 h-5 flex-shrink-0" /> 3. Fator de Atividade
          </h3>
          <div className="bg-white p-4 md:p-8 rounded-[1.25rem] md:rounded-[2rem] border border-slate-100 shadow-sm">
            <div className="flex flex-wrap gap-2 mb-5 md:mb-6 border-b border-slate-100 pb-4 md:pb-6">
              <button type="button" onClick={() => setFormData({...formData, activityCalcMethod: 'auto'})} className={`px-3 md:px-4 py-2 rounded-xl font-bold text-[10px] md:text-xs ${formData.activityCalcMethod === 'auto' ? 'bg-emerald-700 text-white' : 'bg-slate-100 text-slate-600'}`}>Automático</button>
              <button type="button" onClick={() => setFormData({...formData, activityCalcMethod: 'manual'})} className={`px-3 md:px-4 py-2 rounded-xl font-bold text-[10px] md:text-xs ${formData.activityCalcMethod === 'manual' ? 'bg-emerald-700 text-white' : 'bg-slate-100 text-slate-600'}`}>Manual</button>
              <button type="button" onClick={() => setFormData({...formData, activityCalcMethod: 'mets'})} className={`px-3 md:px-4 py-2 rounded-xl font-bold text-[10px] md:text-xs ${formData.activityCalcMethod === 'mets' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600'}`}>METs</button>
            </div>

            {formData.activityCalcMethod === 'auto' && (
              <div className="space-y-5 md:space-y-6">
                <div>
                  <label className="block text-xs md:text-sm font-bold text-slate-800 mb-2 md:mb-3">Rotina de trabalho:</label>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-3">
                    {[
                      ...
                      <label key={item.id} className={`p-2.5 md:p-3 border-2 rounded-xl cursor-pointer font-bold text-center text-[11px] md:text-sm ${formData.routine === item.id ? 'border-emerald-600 bg-emerald-50 text-emerald-800' : 'border-slate-200 bg-slate-50'}`}>
                ...
                <div>
                  <label className="block text-xs md:text-sm font-bold text-slate-800 mb-2 md:mb-3">Horas semanais de CARDIO:</label>
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-2 md:gap-3">
                    {[
                      ...
                      <label key={`cardio-${item.id}`} className={`p-2.5 border-2 rounded-xl cursor-pointer font-bold text-center text-[10px] md:text-xs ${formData.exerciseCardio === item.id ? 'border-blue-500 bg-blue-50 text-blue-800' : 'border-slate-200 bg-slate-50'}`}>
                ...
                <div>
                  <label className="block text-xs md:text-sm font-bold text-slate-800 mb-2 md:mb-3">Horas semanais de FORÇA:</label>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
                    {[
                      ...
                      <label key={`str-${item.id}`} className={`p-2.5 border-2 rounded-xl cursor-pointer font-bold text-center text-[10px] md:text-xs ${formData.exerciseStrength === item.id ? 'border-orange-500 bg-orange-50 text-orange-800' : 'border-slate-200 bg-slate-50'}`}>
            ...
            {formData.activityCalcMethod === 'manual' && (
              <div>
                <label className="block text-xs md:text-sm font-bold text-slate-700 mb-2">Fator (FA)</label>
                <input type="number" step="0.01" name="manualFA" value={formData.manualFA} onChange={handleInputChange} className="w-full md:w-1/2 p-2.5 md:p-3 border-2 border-slate-200 rounded-xl outline-none" />
              </div>
            )}
        ...
        {/* ETAPA 4 */}
        <section className="bg-white p-4 md:p-8 rounded-[1.25rem] md:rounded-[2rem] border border-slate-100 shadow-sm">
          <h3 className="text-base md:text-lg font-black text-slate-800 uppercase italic mb-4 flex items-center gap-2">
            <CheckCircle2 className="text-emerald-700 w-4 h-4 md:w-5 md:h-5 flex-shrink-0" /> 4. Seleção da Equação
          </h3>
          <div className="flex gap-4 md:gap-6 mb-5 md:mb-6">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="radio" name="calculationMode" value="auto" checked={formData.calculationMode === 'auto'} onChange={handleInputChange} className="w-4 h-4 md:w-5 md:h-5 accent-emerald-600" />
              <span className="font-bold text-xs md:text-base">Automática</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="radio" name="calculationMode" value="manual" checked={formData.calculationMode === 'manual'} onChange={handleInputChange} className="w-4 h-4 md:w-5 md:h-5 accent-emerald-600" />
          ...
          {formData.calculationMode === 'manual' && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
              {[
                ...
                <label key={f.id} className={`p-3 md:p-4 border-2 rounded-xl font-black cursor-pointer text-xs md:text-sm ${formData.manualFormula === f.id ? 'border-emerald-600 bg-emerald-50' : 'border-slate-200'}`}>
                  <input type="radio" name="manualFormula" value={f.id} checked={formData.manualFormula === f.id} onChange={handleInputChange} className="mr-2 accent-emerald-600"/>{f.name}
                </label>
              ))}
            </div>
          )}
        ...
        <button type="submit" className="w-full bg-slate-900 hover:bg-emerald-700 text-white font-black py-4 md:py-5 rounded-[1.25rem] md:rounded-[1.5rem] text-sm md:text-lg uppercase tracking-widest transition-all">
